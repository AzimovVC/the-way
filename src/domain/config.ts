...
/**
 * Насколько лифт фокуса (см. focusLift.ts) поднимает лицо круга над его же подставкой, в локальных
 * единицах пути.
 *
 * Выведено из подставки, а не подобрано: обычный день стоит на PLINTH_DEPTH = 6, и лифт — это та же
 * подставка, выросшая под тем кругом, на который смотрят. 8 даёт в фокусе 14 — чуть больше вдвое,
 * то есть разница, которую видно, пока сосед рядом остаётся собой. Выше 16 круг отрывается от
 * подставки настолько, что читается как отдельный предмет, висящий над дорогой.
 */
export const FOCUS_LIFT_PX = 8

/**
 * Радиус затухания лифта, в днях пути.
 *
 * Считается в днях, а не в пикселях экрана, потому что расстояние до фокуса приложение уже знает
 * точно — это разница индексов, на которую поставлен скролл. Расстояние по вертикали дало бы то же
 * число только на прямой дороге: на развороте два дня из разных петель стоят на одной высоте, и
 * поднялись бы оба.
 *
 * Ровно 2 — это «сосед позади и сосед впереди, и больше никто». Число выбрано не на глаз: на
 * smoothstep целый день расстояния даёт ровно половину лифта, так что сосед встаёт посередине
 * между текущим днём и дорогой, а второй по счёту приходится точно на ноль и остаётся плоским.
 * Отсюда три различимые высоты вместо двух — дорога, ступенька, вершина, — и по ним видно не
 * только какой день сейчас, но и куда он едет.
 *
 * Меньше — и сосед почти не отвечает: лифт перескакивает с круга на круг, и переход читается как
 * рывок. Больше — и поднимается полэкрана, а тогда не поднято ничего: поднятость читается только
 * против чего-то неподнятого.
 */
export const FOCUS_LIFT_FALLOFF_DAYS = 2

/**
 * Насколько тело круга сужается к земле — px радиуса на каждый px высоты (см. plinthBody.ts).
 *
 * Существует ради силуэта, а не ради красоты. Подставка той же ширины, что и лицо, даёт контур,
 * который идёт наружу у лица, ещё раз наружу у земли и между ними вваливается внутрь: глаз читает
 * два диска, а не один предмет. Стоит боку пойти внутрь монотонно — и те же две фигуры собираются
 * в одну.
 *
 * 0.22 — это ~12° от вертикали, наклон, который виден, но не превращает круг в каплю. Число
 * привязано к высоте, а не к радиусу, поэтому обычный день (6px) почти не меняется, а поднятый
 * в фокусе (14px) получает заметный конус — сужение приходит вместе с высотой, которая его
 * оправдывает.
 */
export const PLINTH_TAPER_RATIO = 0.22

/**
 * Насколько узким основание может стать, долей от радиуса лица.
 *
 * Ползунок лифта в DevPanel достаёт до 24px, а на такой высоте сужение съело бы почти весь радиус
 * и круг встал бы на иголку. 0.6 — предел, за которым тело перестаёт читаться как стоящее на земле.
 */
export const PLINTH_MIN_BASE_RATIO = 0.6

/**
 * За сколько дней назад лифт фокуса разгорается до полной силы (см. focusLift.ts).
 *
 * Ровно столько же, сколько радиус затухания, и по той же причине: лифт достаёт на два дня, значит
 * и просыпаться должен за два. На сегодня выходит ноль, днём раньше — половина, двумя — полный.
 *
 * Существует, чтобы приём был ответом на действие, а не фоном. В покое дорога стоит на сегодня, и
 * если бы сегодня поднималось само по себе, подъём был бы виден всегда и не значил бы ничего —
 * а сегодня к тому же уже помечено, и ярче: оно единственное неприглушённое, и у него кольцо. Две
 * метки об одном спорят, и высота проигрывает. Лифт поэтому помечает только то, что метки лишено:
 * день в прошлом, на который сейчас смотрят.
 *
 * Гаснет плавно, а не выключается на сегодня: иначе, возвращаясь домой, видишь соседа сегодня
...
/**
 * Насколько глубока лента — и своя половина, и чужая.
 *
 * Неделя, потому что неделя и есть единица времени во всём приложении: Пн–Вс, недельные итоги,
 * значки `Н1`, `Н2`. Лента, показывающая ровно тот отрезок, о котором говорят остальные экраны, не
 * заводит шестой способ мерить время.
 *
 * Число **одно на обе половины**, и это решение, а не экономия. Лента здесь одна, вкладок «Ты» и
 * «Друзья» нет, и две разные глубины в одном списке означали бы, что твой март стоит между её
 * вчера и позавчера. Свой архив не теряется: он и есть дорога, где каждый день — круг, на который
 * можно нажать, плюс архив недель по значкам и экраны месяцев. Лента, уходящая на полгода назад,
 * пересказывала дорогу другими словами.
 *
 * Настройки для человека у этого числа нет и не будет: глубина ленты — не то, о чём он что-то
 * решает, а «показать раньше» на дне списка это уже архив чужой жизни, от которого лента и
 * отделялась.
 */
