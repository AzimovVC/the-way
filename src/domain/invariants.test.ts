 ...
 * такое не ловится: код продолжает работать, просто начинает считать человеку то, чего считать не должен.
...
describe('время суток ни во что не идёт', () => {
  // Точный час читается как обещание, а «утром» опоздать нельзя — но только пока partOfDay не
  // попал в то, что считает. Один импорт отсюда превращает порядок в списке во второе слагаемое счёта.
  const judges = [
...
describe('выходной и заморозка — один предикат', () => {
  ...
  // луну, пишут «под заморозкой», тратят кредит), а не считают по ней.
  ...
  it('nobody else reads day.frozen', () => {
    ...
    expect(readers, 'день считается через isDayExcused, а не по day.frozen').toEqual([])
  })
})
