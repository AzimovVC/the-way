 ...
 * день, день обязан переписывать свой счёт задним числом. Вторая: власть над чужой записью бьёт всегда по
...
export function pairVerdict(circle: Circle, day: Day | undefined, date: string): PairVerdict {
  // Дня нет вовсе — до начала истории или после её конца. Считать нечего.
...
export interface PairProgress {
   ...
   * же логика, что у дел: награда есть, потери нет, накрутить нечем.
   */
  together: number
}
...
/**
 * Те же два числа, но **для строки дня** — без сказуемого и без точки.
 *
 * Карточка дня — список, а не рассказ: в ней строка кружка стоит под названием привычки, и целое
 * предложение там весит больше, чем всё, что оно сообщает. Слова забраны, числа остались, порядок
 * тот же — серия впереди, «за всё время» рядом, равные числа названы один раз.
 *
 * Пустая пара молчит вовсе. «Общий счёт пойдёт с первого дня, который закроете оба» — объяснение
 * механизма в том месте, где человек смотрит на сегодняшний день; само сиденье рядом с названием
 * говорит, что привычка на двоих, а первый общий день скажет о себе сам.
 */
export function pairTally(progress: PairProgress): string {
  const { streak, together } = progress
  if (streak === 0 && together === 0) return ''
  if (streak === 0) return `${together} ${dayWord(together)} всего`
  const run = `${streak} ${dayWord(streak)} подряд`
  return together > streak ? `${run} · ${together} всего` : run
}
