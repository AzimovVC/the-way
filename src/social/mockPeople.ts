...
/**
 * Чьи полки открыты всем.
 *
 * У настоящего человека это его же настройка (`User.habitsPublic`), лежащая на его строке; сюда
 * она вписана руками, потому что своего профиля локально не запросить — никто не спрашивает у
 * приложения о тебе самом, — и без этих двоих правило негде было бы увидеть работающим.
 *
 * Наружу этот список не уходит **никогда**, даже флагом: «он прячет привычки» — это тоже сведение
 * о человеке, и отвечать на него сервер не обязан. Полка либо приехала, либо нет.
 */
const OPEN_SHELVES = new Set(['p-anton', 'p-timur'])

export function shelfIsOpen(personId: string): boolean {
  return OPEN_SHELVES.has(personId)
}
