...
7. **Вход по почте.** Authentication → Sign In / Providers → Email включён, и там же —
   **Minimum password length**. Оно должно быть **не больше 8**: столько обещает подпись под полем
   ...
   ссылка, а приложение спрашивает **цифры кода** — ссылка увела бы человека из установленного на
   ...
   **Шаблонов именно два, и забыть второй легко.** Первое письмо новому человеку идёт по «Confirm
   ...
   пришла сессия, значит внутрь; не пришла, значит спрашиваем код, — но с ссылкой вместо
   `{{ .Token }}` спрашивать будет нечего.

   Длину кода задаёт сервер (Authentication → Email OTP length, 6..10), и приложение её **не
   знает**: поле принимает вилку целиком. Вписанное сюда число однажды разошлось бы с панелью —
   как уже разошлось: восьмизначный код молча обрезался до шести цифр, и человек читал «код не
   подошёл» про код, который подошёл бы.
   ...
   **Свой SMTP без своего домена — это половина решения.** С отправителем вроде
   `onboarding@resend.dev` провайдер пускает письма **только на почту владельца аккаунта**:
   на любой чужой адрес приходит тот же `500 unexpected_failure`, что и от переполненного
   встроенного мейлера, — и выглядит это как поломка, хотя настроено всё верно. Для разработки
   этого хватает; чтобы письма ходили людям, нужен домен, проверенный DKIM и SPF у провайдера.
