...
9. **Вещание.** SQL Editor → [migrations/0008_circle_realtime.sql](./migrations/0008_circle_realtime.sql)
   → Run. Это и есть «ты нажал — она видит»: без него кружок работает, но её галочка появляется
   только при следующем открытии экрана.

   Файл сам печатает, что вещает. Ждём в списке `circle_invites, circle_marks, circles`. Политики
   действуют и здесь — Realtime отдаёт строку только тому, кому её отдала бы обычная `select`.

10. **Почтовый вход выключен.** Authentication → Sign In / Providers → **Email** → off.

   Дверь в приложении одна, и это Google. Выключать провайдер на сервере обязательно, а
...
11. **Вход через Google.** Три места, и пропущенное третье — самая частая причина «ничего не
   происходит».
...
12. **Ключи.** Project Settings → **API Keys** → **Publishable key** (`sb_publishable_…`) и
...
13. `npm run dev` → Профиль → Настройки → Аккаунт.

14. **Проверить политики руками.** SQL Editor → [tests/rls.sql](./tests/rls.sql) целиком → Run.
...
15. **Гифки.** SQL Editor → [migrations/0013_messages.sql](./migrations/0013_messages.sql) → Run.
   Ждём `true · 3 · 3`: RLS включён, три политики (видят двое, пишешь только другу, убирают двое)
   и три триггера (лимит в час, уборка ящика, уборка при разрыве дружбы).

   Поиск идёт прямо с телефона в KLIPY, и ему нужен свой ключ: partner.klipy.com → API Keys →
   `VITE_KLIPY_API_KEY=` в `.env.local`. Тестовый ключ — 100 запросов в час; для живых людей
   попросить там же продакшн-ключ, он бесплатный.

## Что здесь лежит и чего не лежит

`profiles` — имя, ник и три числа, которые и так видны по нику. Читать их может любой вошедший: это и есть карточка, которую открывают по нику.
...
`messages` — гифки между друзьями. В строке только ключ KLIPY, две ссылки и размер — сами
картинки лежат у KLIPY. Ссылки прибиты к его доменам проверкой колонки: телефон получателя грузит
их сам, и ссылка куда угодно ещё сказала бы чужому серверу, кто и когда открыл дорогу. Ящик
живёт неделю и держит не больше 30 штук; уборка идёт на вставке, без планировщика.
